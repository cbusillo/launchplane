...
def search_dokploy_applications(
    *,
    ...
    if page_size < 1 or page_size > 100:
        raise click.ClickException("Dokploy application.search page size must be from 1 to 100.")
    maximum_pages = 10
    ...
    for page_index in range(maximum_pages):
        ...
        if not isinstance(raw_items, list) or not isinstance(total, int) or total < 0:
            raise click.ClickException(
                "Dokploy application.search returned malformed search evidence."
            )
        if expected_total is None:
            expected_total = total
        elif total != expected_total:
            raise click.ClickException(
                "Dokploy application.search total changed during bounded enumeration."
            )
        if len(raw_items) > page_size or len(items) + len(raw_items) > total:
            raise click.ClickException(
                "Dokploy application.search returned an inconsistent result page."
            )
        for item in raw_items:
            ...
            application_id = str(item.get("applicationId") or item.get("id") or "").strip()
            if not application_id:
                raise click.ClickException(
                    "Dokploy application.search returned an application without identity."
                )
            if application_id in application_ids:
                raise click.ClickException(
                    "Dokploy application.search returned duplicate application identity."
                )
        ...
        if not raw_items:
            raise click.ClickException(
                "Dokploy application.search ended before the declared total."
            )
    raise click.ClickException("Dokploy application.search exceeded the bounded page limit.")
