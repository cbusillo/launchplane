...
def release_decision_issue_body(decision: ReleaseReviewDecisionRecord) -> str:
    ...
    if len(body.encode("utf-8")) > 60000:
        raise ValueError("The complete release record exceeds the provider issue size limit.")
    return body
    ...
    *,
    ...
    for page in range(1, 11):
        ...
        for issue in issues:
            ...
            if "pull_request" in issue or not str(issue.get("body") or "").startswith(marker):
                continue
            number = issue.get("number")
            if not isinstance(number, int) or number < 1:
                raise ValueError("Release record number is unavailable.")
            return f"https://github.com/{profile.repository}/issues/{number}"
        if len(issues) < 100:
            break
    else:
