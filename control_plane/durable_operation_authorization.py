...
SUPPORTED_MANAGED_RULE_POLICY_SCHEMA_VERSIONS = frozenset({2, 3})
...
def capture_explicit_action_durable_operation_authorization(
    *,
    identity: LaunchplaneIdentity,
    action: str,
    product: str,
    ...
    try:
        ...
        raise DurableOperationAuthorizationCaptureError(
            "Durable operations require exactly one explicit-action managed authz rule match."
        ) from error
    return DurableOperationAuthorization(
        action=action,
        product=product,
...
def require_single_managed_rule_identity(
    *,
    ...
    if policy.schema_version not in SUPPORTED_MANAGED_RULE_POLICY_SCHEMA_VERSIONS:
...
def require_single_explicit_action_managed_rule_identity(
    *,
    policy: LaunchplaneAuthzPolicy,
    identity: LaunchplaneIdentity,
    action: str,
    ...
    if policy.schema_version not in SUPPORTED_MANAGED_RULE_POLICY_SCHEMA_VERSIONS:
        raise ManagedRuleAuthorizationError(
            "Explicit-action authorization requires schema-v2 or schema-v3 authz policy."
        )
    matches = _matching_managed_rule_identities(
        policy=policy,
        identity=identity,
    ...
    if len(matches) != 1:
        raise ManagedRuleAuthorizationError(
            "Explicit-action authorization requires exactly one matching managed authz rule."
        )
    managed_set_id, managed_rule_id = matches[0]
    return ManagedRuleIdentity(
        managed_set_id=managed_set_id,
...
def require_single_explicit_action_managed_github_id_rule_identity(
    *,
    policy: LaunchplaneAuthzPolicy,
    identity: GitHubHumanIdentity,
    action: str,
    ...
    managed_identity = require_single_explicit_action_managed_rule_identity(
        policy=policy,
        identity=identity,
        action=action,
        product=product,
...
def managed_github_id_rule_allows(
    *,
    ...
    if policy.schema_version not in SUPPORTED_MANAGED_RULE_POLICY_SCHEMA_VERSIONS or github_id < 1:
        return False
    matching_rules = tuple(
        rule
        for rule in policy.github_humans
...
def managed_github_id_explicit_action_rule_allows(
    *,
    policy: LaunchplaneAuthzPolicy,
    github_id: int,
    managed_set_id: str,
    ...
    if len(matching_rules) != 1:
        return False
    rule = matching_rules[0]
    if action not in rule.actions or not rule.github_ids or github_id not in rule.github_ids:
        return False
    return bool(
        rule.allows_scope(
            action=action,
...
def explicit_action_durable_operation_authorization_allows(
    *,
    authorization: DurableOperationAuthorization,
    policy_record: LaunchplaneAuthzPolicyRecord,
) -> bool:
    ...
    *,
    ...
    for rule in rules:
        if rule.managed_set_id is None or rule.managed_rule_id is None:
            continue
        if require_explicit_action and action not in rule.actions:
            continue
        if rule.allows(
            identity=identity,
            action=action,
...
def _managed_rule_allows(
    *,
    ...
    if len(matches) != 1:
        return False
    if require_explicit_action and action not in matches[0].actions:
        return False
    return bool(
        matches[0].allows(
            identity=identity,
