...
ODOO_DRIVER = DriverDescriptor(
    ...
    actions=(
        ...
        _action(
            "prod_backup_verification",
            "Verify prod backup",
            "Run non-destructive integrity and recovery-staging checks against an exact passing prod backup gate.",
            safety="safe_write",
            scope="instance",
            route_path="/v1/drivers/odoo/prod-backup-verification",
            authz_action="odoo_prod_backup_verification.execute",
            writes_records=("backup_gate",),
        ),
        _action(
            "prod_backup_restore_plan",
            "Plan verified prod backup restore",
            "Bind exact backup, verification hashes, artifact identity, and old/new volume authority into a reviewed destructive restore plan.",
            safety="read",
            scope="instance",
            route_path="/v1/drivers/odoo/prod-backup-restore-plan",
            authz_action="odoo_prod_backup_restore_plan.read",
            readiness_requirements=(
                "provider_target",
                "route_binding",
                "runtime_environment",
        ...
        _action(
            "prod_retained_volume_backup_import_plan",
            "Plan retained-volume backup import",
            "Inspect exact retained Odoo volumes read-only and bind source labels, PostgreSQL 17 control metadata, current runtime identity, destination authority, and free-space evidence into a reviewed plan.",
            safety="safe_write",
            scope="instance",
            route_path="/v1/drivers/odoo/prod-retained-volume-backup-import-plan",
            authz_action="odoo_prod_retained_volume_backup_import_plan.execute",
            writes_records=("odoo_prod_retained_volume_backup_import_operation",),
            readiness_requirements=(
                "provider_target",
                "route_binding",
                "runtime_environment",
        ...
        _action(
            "prod_retained_volume_backup_import_apply",
            "Import retained volumes as backup",
            "Clone exact retained Odoo volumes without writing them, capture a standard logical backup and renamed filestore archive, and preserve the recovery-labeled staging clone without target cutover.",
            safety="safe_write",
            scope="instance",
            route_path="/v1/drivers/odoo/prod-retained-volume-backup-import-apply",
            authz_action="odoo_prod_retained_volume_backup_import_apply.execute",
            writes_records=(
                "odoo_prod_retained_volume_backup_import_operation",
                "backup_gate",
            ),
            readiness_requirements=(
                "provider_target",
        ...
        _action(
            "prod_backup_restore_apply",
            "Restore verified prod backup",
            "Restore an exact verified Odoo production backup into a fresh DB volume while preserving the corrupt DB volume and quarantining the old filestore.",
            safety="destructive",
            scope="instance",
            route_path="/v1/drivers/odoo/prod-backup-restore-apply",
            authz_action="odoo_prod_backup_restore_apply.execute",
            writes_records=(
                "odoo_prod_backup_restore_operation",
                "deployment",
                "inventory",
                "release_tuple",
            ),
