...
VeriReelProdBackupGateOperationStatus = Literal["pending", "running", "pass", "fail"]
    ...
    def _validate_record(self) -> "VeriReelProdBackupGateOperationRecord":
        ...
        if self.status in VERIREEL_PROD_BACKUP_GATE_TERMINAL_OPERATION_STATUSES:
            if not self.finished_at:
                raise ValueError("Terminal VeriReel prod backup gate operations require finished_at.")
            if self.status == "pass" and self.error_message:
                raise ValueError(
                    "Passing VeriReel prod backup gate operations must not include error_message."
                )
            if self.status == "fail" and not self.error_message:
                raise ValueError("Failed VeriReel prod backup gate operations require error_message.")
        return self
