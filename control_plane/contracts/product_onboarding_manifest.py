...
class ProductOnboardingManifest(BaseModel):
    ...
    def _normalize_provider_targets(cls, data: object) -> object:
        if not isinstance(data, dict):
            return data
        updated = dict(data)
        if "dokploy_targets" in updated:
            raise ValueError(
                "product onboarding manifest uses obsolete dokploy_targets; use provider_targets"
            )
        updated["provider_targets"] = _normalized_onboarding_targets(
            updated.get("provider_targets", ())
        )
        return updated
