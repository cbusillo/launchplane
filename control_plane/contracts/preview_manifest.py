...
class HarborResolvedPreviewManifest(BaseModel):
    ...
    def _validate_manifest(self) -> "HarborResolvedPreviewManifest":
        if not self.context.strip():
            raise ValueError("resolved Harbor preview manifest requires context")
        if not self.baseline_channel.strip():
            raise ValueError("resolved Harbor preview manifest requires baseline_channel")
        if not self.baseline_release_tuple_id.strip():
            raise ValueError("resolved Harbor preview manifest requires baseline_release_tuple_id")
        if not self.resolved_manifest_fingerprint.strip():
            raise ValueError("resolved Harbor preview manifest requires resolved_manifest_fingerprint")
        if not self.source_map:
            raise ValueError("resolved Harbor preview manifest requires source_map")
        return self
