...
| Tenant repository classification | `launchplane_tenant_repository_classifications` | DB-backed Launchplane classification record | Classifies repositories by numeric ID as engineering or tenant UI for pure merge eligibility. Revisions are immutable with CAS operator recovery. Pure evaluation is separate from scheduler merge train policy. |
