...
| Local-operator write bearer token | `LAUNCHPLANE_LOCAL_OPERATOR_TOKEN`, optional `LAUNCHPLANE_LOCAL_OPERATOR_SUBJECT`, optional `LAUNCHPLANE_LOCAL_OPERATOR_TOKEN_LABEL` | Bootstrap env or platform secret | Shared by the Launchplane service and trusted local owner automation for product-config plan/apply only. Store it outside repository config and keep it distinct from read-only terminal-agent credentials. |
