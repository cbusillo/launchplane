...
| Authz bootstrap                           | `LAUNCHPLANE_POLICY_TOML`, `LAUNCHPLANE_POLICY_B64`, `LAUNCHPLANE_POLICY_FILE`                                                                                                  | Minimal bootstrap env/file       | Root of trust for first start and DB policy repair only. Live product/workflow grants are DB-backed authz policy records. During managed-policy migration, existing name-only rules remain readable, but every new GitHub Actions grant written through the service must include immutable repository and owner IDs. |
