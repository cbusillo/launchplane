...
| Tenant repository classification | `launchplane_tenant_repository_classifications` | DB-backed Launchplane classification record | Classifies repositories by numeric ID as engineering or tenant UI for pure merge eligibility. Revisions are immutable with CAS operator recovery and durable apply reservations; filesystem records are rehearsal/import input, not shared authority. Pure evaluation is separate from scheduler merge train policy. |
