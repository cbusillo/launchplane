...
continues. Deploying this code does not activate v2, migrate policy, add a grant,
or reuse historical Owner
...
generator contributions, trusted stored-evidence semantics, and policy/evaluation
schema versions. It also binds
...
`previous_path` and evidence for its origin path. Missing or invalid
...
per-generator contributions used by scoped decision binding. Mapping changes
...
remains pending explicit rollout qualification.
