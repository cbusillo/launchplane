...
`uv run launchplane authz-policies grant-workflow --service-url ...
--repository-id ... --repository-owner-id ... --dry-run`
...
Pass `--repository-id` and `--repository-owner-id` when removing an ID-bound
rule; omit both only when retiring a name-only compatibility rule.
...
Apply returns a conflict when the active record changed after planning, and an
exact removal cannot delete the final policy-administration principal.
