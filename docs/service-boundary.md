...
[historical-completion preflight](merge-train-historical-completion.md)
...
perform a provider mutation. Native PostgreSQL also supports explicit
`mutate=true` with an idempotency key: fresh GET proof and current DB authority
precede one atomic observation-only successor, exact predecessor retirement,
controller release and completed receipt. It creates no admission or ordinary
outcome and performs no provider write. Current-authorized replay is independent
of later controller work. Other backends cannot apply this recovery, and
ordinary-agent execution cannot invoke this legacy-only selector.
Without the selector, each call advances at most one safe phase from DB-backed records and
...
the same scoped inspection authorization because the
