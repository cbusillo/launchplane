...
Enrollment can separately inspect the configured App, exact immutable repository
identity, installation and closed permission ceiling without minting a token. The
result is non-secret evidence for the later atomic enrollment candidate builder;
the provider reads complete before any enrollment write transaction begins.
