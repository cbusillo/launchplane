...
Issue `#2239` establishes the recovery root for a total authorization lockout.
Launchplane persists only OpenSSH security-key public verification material and
accepts only `sk-ssh-ed25519@openssh.com` SSHSIG envelopes in the fixed
`launchplane.authorization-recovery.v1` namespace. It never receives, stores,
or derives a private key. Two independently labelled custody slots must be
active before a recovery challenge can be prepared or applied; pending keys
must first prove possession with an SSHSIG over the server-issued proof
challenge. Revocation is checked before signature verification and cannot leave
fewer than two independently custodied active keys.

The signed recovery request is canonical exact JSON generated by the service.
It binds the service identity, operation, nonce, issue/expiry timestamps, exact
active policy record/revision/digest, immutable intended GitHub administrator
ID, fixed managed set/rule IDs, and server-computed plan digest. Apply accepts
only a challenge ID, recovery key ID, and SSHSIG; it reconstitutes every signed
byte server-side, consumes the single-use challenge, re-plans the bounded rule,
and rejects policy drift, replay, expiry, revoked keys, or CAS/read-back
ambiguity. Audit evidence exposes only bounded IDs, key fingerprints, statuses,
and the necessary intended GitHub ID; it never exposes a signature, public-key
line, session, selector, or policy payload.
...
Actions, agents, local-admin identities, provider mutations, secrets, or a
general policy executor. Initial bootstrap writes a monotonic completion record;
after that record is complete, `LAUNCHPLANE_GITHUB_OAUTH_BOOTSTRAP_ADMIN_EMAILS`
cannot elevate a session even if a later policy becomes empty. Ordinary DB-native
administration must normalize the recovered policy.
