...
cutover. The current service allows bootstrap-email role elevation only while
configured bootstrap emails have an explicitly wired bootstrap-state provider
and that provider reports no completed recovery-bootstrap record. Provider
failure or missing service wiring fails closed for email elevation; explicit DB
denial then wins on login and session revalidation. Legacy human rules retain
their existing runtime matching semantics until a reviewed migration replaces
wildcard or implicit selectors, while all newly reconciled managed human rules
require explicit roles, explicit principals, exact selectors, and immutable IDs
for sensitive access. Changed applies must also retain a policy administrator
independent from the applying identity. Break-glass must use a separate
credential and approval boundary, bind
...
`key-type key-blob`; comments are ignored, and fingerprints are SHA-256 over the
decoded key blob. Only `sk-ssh-ed25519@openssh.com` is accepted for recovery
custody keys. SSHSIG verification shells out to `ssh-keygen -Y verify` with a
fixed signer identity, the fixed `launchplane.authorization-recovery.v1`
namespace, bounded signature size and timeout, and no stderr leakage.
Signatures and canonical signed bytes are never persisted.

The signed recovery request is canonical exact JSON generated by the service.
It binds the service identity, exact operation variant, cryptographic nonce,
issue/expiry timestamps, active policy record/revision/digest, candidate record
and policy digest, immutable intended GitHub administrator ID, fixed managed
set/rule IDs, signing key ID, and signing key canonical fingerprint. Rotation
also binds the selected compromised key plus replacement key ID, replacement
...
signing key active/fingerprint, two independent custody slots, exact expected
active policy record, exact candidate digest, and operation-specific invariants.
It then supersedes/writes policy, consumes the challenge, completes initial
bootstrap if requested, activates/revokes rotation keys if requested, writes
bounded audit/result evidence, and inserts an operator-alert outbox row all at
once. It commits all of those changes or none, and returns deterministic
applied, adopted, conflict, stale, replay, or reconciliation-required status.
...
if a later policy becomes empty. Ordinary DB-native administration must
normalize the recovered policy in a later reviewed phase.
