...
- Before replacing website-bootstrap settings, read the current persisted payload
  ...
  the lane from the product profile, and returns website intent plus record
  metadata. It does not return config parameters, addon settings, or secrets, and
  does not contact the runtime provider. Missing records return 404.
...
- Invalid signatures, missing deliveries, and malformed payloads stop at the
  ...
  logins.
- Unified tenant admission reads current numeric repository classification and
  exact candidate identity. Matching tenant UI candidates return `eligible`;
  engineering retains its normal flow. Missing, ambiguous, or drifted
  classification stays blocked. Technical checks and controller fences remain
