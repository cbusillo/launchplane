...
class _AuthzPolicyStore:
    ...
    def compare_and_write_authz_policy_record(
        ...
        *,
        ...
        if len(active_records) != 1 or active_records[0] != expected_record:
            return False
        self.records = tuple(
        ...
        self.records = (replacement_record,) + self.records
        return True
...
class AuthzGrantServiceTests(unittest.TestCase):
    ...
    def test_write_rejects_stale_active_policy(self) -> None:
        ...
        with self.assertRaisesRegex(AuthzPolicyConflictError, "active authz policy changed"):
            write_github_actions_authz_policy_grant(
                record_store=store,
                request=_grant_request(),
                identity=_identity(),
