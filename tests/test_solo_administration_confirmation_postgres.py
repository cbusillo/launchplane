        ...
        with _isolated_postgres_database() as database_url:
            ...
            try:
                wrong_scope = _issue_record(
                    idempotency_scope_sha256="1" * 64,
                    idempotency_key_sha256="2" * 64,
                )
                wrong_identity = _issue_record(
                    github_id=456,
                    idempotency_scope_sha256="d" * 64,
                    idempotency_key_sha256="4" * 64,
                )
                exact = _issue_record()
                for candidate in (wrong_scope, wrong_identity):
                    store.issue_solo_administration_confirmation(candidate)
                    store.consume_solo_administration_confirmation(
                        confirmation_id=candidate.confirmation_id,
                        active_policy_record_id=candidate.active_policy_record_id,
                        active_policy_revision=candidate.active_policy_revision,
                        active_policy_sha256=candidate.active_policy_sha256,
                        candidate_policy_sha256=candidate.candidate_policy_sha256,
                        candidate_administrator_quorum=1,
                        candidate_distinct_human_administrator_count=1,
                        reviewed_plan_sha256=candidate.reviewed_plan_sha256,
                        human_session_id_sha256=candidate.human_session_id_sha256,
                        github_id=candidate.github_id,
                        idempotency_scope_sha256=candidate.idempotency_scope_sha256,
                        idempotency_key_sha256=candidate.idempotency_key_sha256,
                        acknowledgement_sha256=candidate.acknowledgement_sha256,
                ...
                self.assertFalse(
                    store.has_consumed_solo_administration_confirmation(
                        ...
                        idempotency_scope_sha256=exact.idempotency_scope_sha256,
                    )
                )
                store.issue_solo_administration_confirmation(exact)
                store.consume_solo_administration_confirmation(
                    confirmation_id=exact.confirmation_id,
                    active_policy_record_id=exact.active_policy_record_id,
                    active_policy_revision=exact.active_policy_revision,
                    active_policy_sha256=exact.active_policy_sha256,
                    candidate_policy_sha256=exact.candidate_policy_sha256,
                    candidate_administrator_quorum=1,
                    candidate_distinct_human_administrator_count=1,
                    reviewed_plan_sha256=exact.reviewed_plan_sha256,
                    human_session_id_sha256=exact.human_session_id_sha256,
                    github_id=exact.github_id,
                    idempotency_scope_sha256=exact.idempotency_scope_sha256,
                    idempotency_key_sha256=exact.idempotency_key_sha256,
                    acknowledgement_sha256=exact.acknowledgement_sha256,
                    secret_sha256=exact.secret_sha256,
                    terminal_at="2026-08-31T12:01:00Z",
                )
                self.assertTrue(
                    store.has_consumed_solo_administration_confirmation(
