...
class OrdinarySnapshotReaderTests(unittest.TestCase):
    ...
    def test_identity_drift_stops_before_rule_or_role_requests(self) -> None:
        self.response["data"]["repository"]["ref"]["target"]["oid"] = "different-base"
        inner, transport = self.transport()
        with self.assertRaisesRegex(OrdinaryAgentProviderEvidenceError, "snapshot_base_changed"):
            read_ordinary_controller_snapshot(
                transport=transport,
                request=self.request,
                repository_owner_id=202,
