...
class OdooStableBootstrapOperationRecordTests(unittest.TestCase):
    ...
    def test_schema_v2_operation_requires_matching_authorization_provenance(self) -> None:
        ...
        record = OdooStableBootstrapOperationRecord.model_validate(
            {
                **_operation_payload(),
                "schema_version": 2,
    ...
    def test_cancelled_operation_is_terminal_and_requires_cancellation_evidence(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "requires cancellation evidence"):
            OdooStableBootstrapOperationRecord.model_validate(
                {
                    **_operation_payload(),
                    "schema_version": 2,
        ...
        self.assertEqual(record.cancellation.reason, "Operator cancelled pending destructive work.")
            ...
            try:
                for store in stores:
                    with self.subTest(store=type(store).__name__):
                        pending_record = OdooStableBootstrapOperationRecord.model_validate(
                            {
                                **_operation_payload(),
                                "schema_version": 2,
                                "authorization": durable_operation_authorization_payload(
                        ...
                        self.assertIsNone(
                            store.claim_next_odoo_stable_bootstrap_operation_record(
    ...
    def test_filesystem_cancel_and_claim_are_mutually_exclusive(self) -> None:
        with TemporaryDirectory() as temporary_directory_name:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                ...
                cancellation_committed = cancel_future.result(timeout=5)
                claimed_record = claim_future.result(timeout=5)
            loaded = store.read_odoo_stable_bootstrap_operation_record(pending_record.operation_id)

        self.assertNotEqual(cancellation_committed, claimed_record is not None)
        self.assertEqual(
            loaded.status,
            "cancelled" if cancellation_committed else "running",
        )
